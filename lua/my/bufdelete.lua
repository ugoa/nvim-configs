...
local function buf_kill(target_buffers, switchable_buffers, force, wipeout)
  ...
  if not force then
    for bufnr, _ in pairs(buf_is_deleted) do
      ...
      elseif bo[bufnr].buftype == "terminal" and fn.jobwait({ bo[bufnr].channel }, 0)[1] == -1 then
        ...
        if choice ~= "i" and choice ~= "I" then buf_is_deleted[bufnr] = nil end
      end
    end
  end
  ...
  local windows = vim.tbl_filter(
    function(win) return buf_is_deleted[api.nvim_win_get_buf(win)] ~= nil end,
    api.nvim_list_wins()
  )
...
local function find_buffer_with_pattern(pat)
  for _, bufnr in ipairs(api.nvim_list_bufs()) do
    if api.nvim_buf_is_valid(bufnr) and api.nvim_buf_get_name(bufnr):match(pat) then return bufnr end
  end
end
...
local function get_buffer_handle(buffer_or_pat)
  ...
  if bufnr ~= nil and api.nvim_buf_is_valid(bufnr) then return bufnr end
end
...
local function get_target_buffers_from_range(left, right)
  ...
  for i = left, right do
    if api.nvim_buf_is_valid(i) then target_buffers[#target_buffers + 1] = i end
  end
...
local function get_target_buffers(buffers)
  if type(buffers) ~= "table" then return { get_buffer_handle(buffers) } end
